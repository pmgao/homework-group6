...
本学期做其它的某个项目时，碰到了一个难点：便是CUDA平台上的大整数运算实现，众所周知CUDA平台所支持的数据宽度大多与原生的C++一致，只能支持到64比特(虽然自CUDA 11.5版本之后开始支持128比特宽度，但128比特寄存器的读取速度目前仍然比不上32比特寄存器，速度较慢)，但我的难点主要是该平台上的大整数模加、模减与模乘的快速实现，而通用的CUDA平台第三方大整数运算库CGBN虽然实现速度已经足够快，但存在着一定的缺陷：
